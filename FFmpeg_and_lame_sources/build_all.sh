...
./configure --target-os=linux --arch=x86 --cpu=i686 --cross-prefix=i686-linux-android- --enable-cross-compile $FEATURES --disable-symver --disable-doc --disable-htmlpages --disable-manpages --disable-podpages --disable-txtpages --disable-ffplay --disable-ffprobe --disable-ffserver --enable-gpl --enable-version3 --disable-amd3dnow --disable-amd3dnowext --disable-asm --enable-yasm --enable-pic --prefix=$PREFIX --extra-cflags='-std=c99 -O3 -Wall -fpic -pipe   -DANDROID -DNDEBUG  -march=atom -msse3 -ffast-math -mfpmath=sse' --extra-ldflags='-lm -lz -Wl,--no-undefined -Wl,-z,noexecstack'
...
# copy builds
mkdir builds
cp -v $BUILD_DIR/armv7a/bin/ffmpeg builds/ffmpeg_armv7a
cp -v $BUILD_DIR/neon/bin/ffmpeg builds/ffmpeg_armv7a-neon
cp -v $BUILD_DIRx86/bin/ffmpeg builds/ffmpeg_x86
